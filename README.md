...
### How to use?
 ...
 - **Filter/process data**.
 The purpose of filtering is to clean grabbed data so that visualization or other analysis is more accurate,
 e.g. you might want to exclude commits related to project documentation or commits generated by CI.
 It also might be useful to write custom analysis on grabbed data (similar to writing database query).
...
#### Filter/process data
Use ``Main menu -> VCS -> Code History Mining`` or ``alt+shift+H`` to open plugin popup,
select one the grabbed files and choose ``Open Script Editor``.
