...
import static history.ChangeEventsExtractor.Util.*
...
class ChangeEventsExtractor {
	...
	Collection<ChangeEvent> fileChangeEventsFrom(Commit commit) {
		try {
			...
			commit.changes.collect { Change change ->
				new ChangeEvent(commitInfo, CommitFileMunger.fileChangeInfoOf(change, project, false), ElementChangeInfo.EMPTY)
			}
	...
	Collection<ChangeEvent> changeEventsFrom(Commit commit) {
		try {
			...
			commit.changes.collectMany { Change change ->
				def fileChangeInfo = CommitFileMunger.fileChangeInfoOf(change, project)
				withDefault([null], CommitMethodMunger.elementChangesOf(change, project)).collect{
	...
	private static class CommitFileMunger {
	...
	private static class CommitMethodMunger {
